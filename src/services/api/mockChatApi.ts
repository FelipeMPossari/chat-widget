...
export class MockChatApi implements IChatApi {
    ...
    private currentUserMember?: XChannelChatMemberDto;
    ...
    async bootstrap(request: BootstrapRequest): Promise<BootstrapResponse> {
        ...
        this.currentUserMember = authenticated
            ? {
                Id: 1,
                UserChatId: 1,
        ...
        return {
            ...
            currentUser: this.currentUserMember
                ? mapMemberDtoToChatMember(this.currentUserMember)
                : undefined,
            settings: {
    ...
    async retryMessage(
        ...
        const index = messages.findIndex(
            (message) => readDtoValue<string>(message, 'MessageGuid') === messageGuid
        );
        ...
        const nextMessages = messages.map((message) =>
            readDtoValue<string>(message, 'MessageGuid') === messageGuid
                ? {
                    ...message,
                    Status: 'pending',
                    Error: undefined,
        ...
        this.messagesByConversation.set(chatGuid, nextMessages);
        this.touchConversation(
            chatGuid,
            readDtoValue<string>(nextMessages[index], 'Body') ??
            readDtoValue<XChannelMessageDto['Attachment']>(nextMessages[index], 'Attachment')?.FileName
        );
...
function createMessageDto(
    ...
    return {
        ...
        SenderUser: {
            Id: params.senderId,
            UserChatId: params.senderUserChatId,
